...
async function downloadMedia(url, type) {
  try {
    ...
    let mediaUrls = [];
    let fileFormat = "bin"; // Default format
    ...
    for (const mediaUrl of mediaUrls) {
      try {
        const mediaResponse = await axios.get(mediaUrl.url, {
        ...
        logger.error(
          `Error downloading ${mediaUrl.filename}: ${downloadError.message}`
        );
      }
    }
  ...
  try {
    ...
    for (const { media, isDocument, filename, fileSize } of downloadedMedia) {
      try {
        ...
        await message.reply(
          `⚠️ Error sending ${filename}. Trying to send as document...`
        );
        ...
        try {
          ...
          if (sent) {
            ...
            await message.reply(
              `✅ ${filename} sent successfully as document.`
            );
          } else {
          ...
          logger.error(
            `Error sending ${filename} as document: ${docError.message}`
          );
          await message.reply(
            `❌ Failed to send ${filename}. You may need to download it manually.`
          );
        }
      }
    }
...
export const ytdl = async (message, args) =>
  handleDownload(message, args, "ytdl");
export const ytmp4 = async (message, args) =>
  handleDownload(message, args, "ytmp4");
export const ytmp3 = async (message, args) =>
  handleDownload(message, args, "ytmp3");
export const spotify = async (message, args) =>
  handleDownload(message, args, "spotify");
export const fbdl = async (message, args) =>
  handleDownload(message, args, "fbdl");
export const igdl = async (message, args) =>
  handleDownload(message, args, "igdl");
