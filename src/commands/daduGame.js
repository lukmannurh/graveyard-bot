...
export const dadu = async (message, args) => {
  try {
    ...
    if (mentions.length !== 1) {
      await message.reply(
        "Silakan tag satu peserta untuk bermain dadu. Contoh: .dadu @peserta"
      );
      return;
    }
    ...
    await chat.sendMessage(
      `@${opponent.number} Anda ditantang bermain dadu oleh @${challenger.number}. Ketik Y untuk menerima atau N untuk menolak.`,
      {
...
export const handleDaduResponse = async (message) => {
  try {
    ...
    const gameId = Array.from(activeGames.entries()).find(
      ([id, game]) =>
        game.opponent === sender.id._serialized &&
        game.state === "waiting_confirmation"
    )?.[0];
    ...
    if (message.body.toLowerCase() === "y") {
      ...
      await chat.sendMessage(
        "Permainan dadu dimulai! Silakan pilih:\n1. Ganjil\n2. Genap",
        {
          mentions: [
            await chat.getContactById(game.challenger),
            await chat.getContactById(game.opponent),
          ],
        }
      );
...
export const handleDaduChoice = async (message) => {
  try {
    ...
    const gameId = Array.from(activeGames.entries()).find(
      ([id, game]) =>
        (game.challenger === sender.id._serialized ||
          game.opponent === sender.id._serialized) &&
        game.state === "choosing"
    )?.[0];
...
const handleTimeoutChoices = async (chat, gameId) => {
  ...
  if (!game.choices[game.challenger] && !game.choices[game.opponent]) {
    await chat.sendMessage(
      "Kedua pemain tidak memberi respon. Permainan dibatalkan."
    );
  } else if (!game.choices[game.challenger]) {
    await chat.sendMessage(
      `@${challenger.number} tidak memberi respon. @${opponent.number} menang!`,
      {
    ...
    await chat.sendMessage(
      `@${opponent.number} tidak memberi respon. @${challenger.number} menang!`,
      {
        mentions: [challenger, opponent],
      }
    );
